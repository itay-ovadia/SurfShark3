...
public class MainActivity extends AppCompatActivity {
    ...
    /*INIT FUNCTIONS*/
    private void initMatrixSharks() {
        main_MATRIX_sharks = new AppCompatImageView[ROW_COUNT][COLUMN_COUNT];
        for (int i = 0; i < ROW_COUNT; i++) {
            for (int j = 0; j < COLUMN_COUNT; j++) {
                String name = "main_IMG_shark_" + (i + 1) + "_" + (j + 1);
                System.out.println(name);
                int id = getResources().getIdentifier(name, "id", getPackageName());
                AppCompatImageView shark = findViewById(id);
                main_MATRIX_sharks[i][j] = shark;
            }
        }
    }
    ...
    private void initViews() {
        ...
        gameManager.initMatrix(main_MATRIX_sharks);
    }
    /*INIT FUNCTIONS*/

    private void visibility(int direction) {
        int place = gameManager.getCurrentPlace();
        main_IMG_surfer[place].setVisibility(View.VISIBLE);
        System.out.println(place + "," + direction);
        main_IMG_surfer[place + direction].setVisibility(View.INVISIBLE);
    }
    ...
    private void findPlanes() {
        main_IMG_surfer = new AppCompatImageView[]{
                findViewById(R.id.main_IMG_surfer),
                findViewById(R.id.main_IMG_surfer2),
                findViewById(R.id.main_IMG_surfer3),
                findViewById(R.id.main_IMG_surfer4),
                findViewById(R.id.main_IMG_surfer5)
        };
    }
